...
Accept the offer to buy the inscription `INSCRIPTION_ID` for `AMOUNT` offered in `PSBT` using:

```
ord wallet offer accept --inscription <INSCRIPTION_ID> --amount <AMOUNT> --psbt <PSBT>
```

Accept the offer to buy the rune balance `DECIMAL:RUNE` offered in `PSBT` using:
